...
- `minigraphnode` : `int` - minigraph node ID if the queried region is based on a minigraph node. This will color the sequence tube map tracks based on the Point Cloud Local Ancestry Inference colors of associated minigraph node
- `format` - `str` - what to return: "svg" (the drawing document, the default and unchanged) or "bands" (the band payload — a JSON header and a binary body, specified in docs/band-format.md)

#### Output

`seqtubemap` directly returns the Sequence Tube Map layout of the requested region in svg format. If the requested region is based on the region of a minigraph node, recording the minigraph node ID in `minigraphnode` will color the sequence tube map tracks based on the Point Cloud Local Ancestry Inference colors of this minigraph node